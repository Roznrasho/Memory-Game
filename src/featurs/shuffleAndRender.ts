...
const shuffleCarde = (cards: Card[]): void => {                      // Shuffles an array of cards in place using the Fisher-Yates algorithm.
  for (let i = cards.length - 1; i > 0; i--) {                      // Loop over the array from the last element to the second element
...
function renderCards(
    ...
    cards.forEach((card) => { 
        ...
        cardElement.className = `card ${card.isFlipped || card.isMatched ? "flipped" : ""}`; // Set the class name based on the card's state
